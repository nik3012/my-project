function [options_out, T] = bmmo_set_model_configuration(options_in)
%function [options_out, T] = bmmo_set_model_configuration(options_in)
%
...
seq_1L_MIKA = {'SUSD_1L_monitor','MIKA','BAO','INTRAF'};
seq_1L_MIKA_SUSD = {'SUSD_1L','MIKA','BAO','INTRAF'};
seq_2L_default = {'WH','SUSD_2L_monitor','MI','BAO','INTRAF'};
seq_2L_MIKA = {'WH','SUSD_2L_monitor','MIKA','BAO','INTRAF'};
seq_2L_SUSD = {'WH_SUSD','MI','BAO','INTRAF'};
seq_2L_SUSD_MIKA = {'WH_SUSD','MIKA','BAO','INTRAF'};
submodel_sequence = {seq_1L_default;seq_1L_MIKA;seq_1L_SUSD;seq_1L_MIKA_SUSD;...
    seq_2L_default;seq_2L_MIKA;seq_2L_SUSD;seq_2L_SUSD_MIKA};
...
for i = 1:length(submodel_sequence)
    cm_contents.OR = {'WH', 'SUSD', 'MIX', 'MIY', 'INTERF', 'INTRAF', 'KA_POLY_NONLINEAR'};
    for j = 1:length(submodel_sequence{i})
        switch submodel_sequence{i}{j}
            case 'MIKA'
                cm_contents.MIKA = {'MIX', 'MIY', 'KA_POLY'};
            case {'SUSD_1L', 'SUSD_1L_monitor'}
                cm_contents.SUSD_1L = {'SUSD', 'MIX', 'MIY', 'INTERF', 'INTRAF', 'KA_POLY_NONLINEAR'};
